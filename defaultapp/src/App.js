import React from 'react';
import 'animate.css';
import Setup from './1-useState/setup/2-useState-basics';
// import Setup from './1-useState/setup/3-useState-array';
// import Setup from './1-useState/setup/4-useState-object';
// import Setup from './1-useState/setup/5-useState-counter';
// import Setup from './1-useState/final/1-error-example';
// import Setup from './2-useEffect/setup/1-useEffect-basics';
// import Setup from './2-useEffect/setup/2-useEffect-cleanup';
// import Setup from './2-useEffect/final/3-useEffect-fetch-data';
// import Setup from'./3-conditional-rendering/setup/1-multiple-returns';
// import Setup from'./3-conditional-rendering/setup/2-short-circuit';
// import Setup from'./3-conditional-rendering/setup/3-show-hide';
// import Setup from './4-forms/setup/1-controlled-inputs';
// import Setup from './4-forms/setup/2-multiple-inputs';
// import Setup from './5-useRef/setup/1-useRef-basics';
// import Setup from './6-useReducer/setup';
// import Setup from './7-prop-drilling/setup/1-prop-drilling';
// import Setup from './8-useContext/setup/1-context-api';
// import Setup from './9-custom-hooks/setup/1-fetch-example';
// import Setup from './10-prop-types/setup';
// import Setup from './10-prop-types/setup/Product';
const App = () => {

  return (
    <div className='container'>
        
      <Setup/>
        
    </div>
    
  )
}

export default App;